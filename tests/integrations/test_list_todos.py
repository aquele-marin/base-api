...
@pytest.mark.asyncio
@test_steps("create_test_todos", "list_all_todos", "verify_response_structure", "verify_all_todos_returned")
...
@pytest.mark.asyncio
@test_steps("create_todos_with_different_statuses", "filter_by_status", "verify_filtered_results")
...
@pytest.mark.asyncio
@test_steps("create_todos_with_different_priorities", "filter_by_priority", "verify_filtered_results")
...
async def test_list_todos_with_combined_filters(test_client: AsyncClient):
    ...
    response = await test_client.get("/api/v1/todos?status=pending&priority=medium&limit=10")
