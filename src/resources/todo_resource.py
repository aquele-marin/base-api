...
class TodoResource:
    ...
    async def create(self, request: TodoCreateRequest) -> TodoResponse:
        """Cria um novo TODO"""
        priority_value = request.priority.value if isinstance(request.priority, TodoPriorityEnum) else request.priority
    ...
    async def update(self, todo_id: UUID, request: TodoUpdateRequest) -> TodoResponse:
        ...
        status_value = request.status.value if request.status and isinstance(request.status, TodoStatusEnum) else None
        priority_value = request.priority.value if request.priority and isinstance(request.priority, TodoPriorityEnum) else None
    ...
    async def update_status(self, todo_id: UUID, request: TodoStatusUpdateRequest) -> TodoResponse:
        ...
        status_value = request.status.value if isinstance(request.status, TodoStatusEnum) else request.status
