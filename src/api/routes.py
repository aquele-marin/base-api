...
async def create_todo(
    todo_data: TodoCreateRequest, resource: TodoResource = Depends(get_todo_resource)
):
    """Cria um novo TODO"""
    try:
...
async def get_todos(
    ...
    priority: Optional[TodoPriorityEnum] = Query(
...
async def get_todo_stats(resource: TodoResource = Depends(get_todo_resource)):
...
async def get_todo(todo_id: UUID, resource: TodoResource = Depends(get_todo_resource)):
...
async def delete_todo(
    todo_id: UUID, resource: TodoResource = Depends(get_todo_resource)
):
    """Deleta um TODO"""
    try:
