...
class TodoCreateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=1000, description="Descrição detalhada do TODO"
    )
    priority: TodoPriorityEnum = Field(
        TodoPriorityEnum.MEDIUM, description="Prioridade do TODO"
    )
...
class TodoUpdateRequest(BaseModel):
    ...
    title: Optional[str] = Field(
        None, min_length=1, max_length=200, description="Novo título do TODO"
    )
    description: Optional[str] = Field(
        None, max_length=1000, description="Nova descrição do TODO"
    )
    priority: Optional[TodoPriorityEnum] = Field(
        None, description="Nova prioridade do TODO"
    )
    status: Optional[TodoStatusEnum] = Field(None, description="Novo status do TODO")
    due_date: Optional[datetime] = Field(
        None, description="Nova data de vencimento do TODO"
    )
